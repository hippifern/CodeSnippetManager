...
type DataType = {
  ...
  optimisations: optimisationType[];
};

export const Dashboard = () => {
  const [currentUser, setCurrentUser] = useState<User>();
  const [selectedSnippet, setSelectedSnippet] = useState<snippetType>();
  const [selectedOptimisation, setSelectedOptimisation] =
    useState<optimisationType>();
  ...
  const saveDataToState = (d: DataType) => {
    setData(d);
  ...
  function selectSnippet(sid: number) {
    if (!data) return null;
    ...
    data.optimisations.forEach((opti) => {
  ...
  const addNewSnippet = (uid: string) => {
  ...
  const deleteSelectedSnippet = (sid: number) => {
  ...
  const updateSelectedSnippet = (
    ...
    if (!currentUser || !selectedSnippet) return null;
    if (!data) return null;
    if (!sid) return null;
    ...
    if (keyToChange === "tags") {
      ...
      setSelectedSnippet((prev) => (prev ? { ...prev, tags } : prev));
    } else {
      setSelectedSnippet((prev) =>
        prev ? { ...prev, [keyToChange]: valueToChange } : prev,
      );
    }
    const selectedId = data.snippets.findIndex((snip) => {
      return snip.id === sid;
  ...
  if (selectedSnippet && selectedOptimisation && currentUser) {
    return (
      <div className="outer-container bg-emerald-300 h-screen w-screen p-6">
        <div className="inner-container bg-white h-full w-full rounded-2xl shadow-emerald-400 shadow-md flex p-8 flex-col">
          ...
          <div className="main-content w-full h-full flex py-8 ">
            ...
            {selectedSnippet && selectedOptimisation ? (
                  ...
                  <CodeBlock
                    value={
                      selectedOptimisation
                        ? selectedOptimisation.improved_code
                        : ""
                    }
                    updateSelectedSnippet={updateSelectedSnippet}
                    keyToChange={"code"}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        deleteSelectedSnippet(
                          selectedSnippet ? selectedSnippet.id : -1,
                        )
                      }
                    />
                  </div>
